#Exercício Python 058: Melhore o jogo do DESAFIO 028 onde o computador vai "pensar" em um número entre 0 e 10. Só que agora o jogador vai tentar adivinhar até acertar, mostrando no final quantos palpites foram necessários para vencer.

from random import randint
from time import sleep
opc = randint(0, 10)
palp = int(input('Acabei de pensar em um número entre 0 e 10. Tente adivinhar: '))
cont = 1
print('')
print('Pensando...')
sleep(0.5)
print('')
while palp != opc:
    cont += 1
    palp = int(input(f'Errou, tente novamente: '))
print(f'Parabéns, eu pensei no número {opc}! Para me vencer foram necessários {cont} palpite(s)')